...
def hashrate_share(mining_hw, btc):
	...
	for row in btc.itertuples():
		...
		if row.avg_hashrate == 0 or (np.abs(row.change_avg_hashrate) / row.avg_hashrate) < 0.01:
...
def main():
	...
	x = [i%14 for i in range(len(btc[btc['date'] > pd.to_datetime('2011-10-01')]['date'].values))]
	tmp = btc[btc['date'] > pd.to_datetime('2016-10-01')]['block_count']
	y = [tmp.iloc[i::14] for i in range(14)]
